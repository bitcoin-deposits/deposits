...
existing miniscript fragments are accepted as parse-time aliases and desugar to the n-ary `and`, `or`, `thresh` above; the wrappers (`a:`, `s:`, `c:`, `v:`, `t:`, `d:`, `j:`, `n:`, `l:`, `u:`) desugar to no-ops. the miniscript combinators `pk`, `older`, `after`, `sha256`, `hash256`, `ripemd160`, `hash160` retain their meaning; their semantic content in the calculus is `prove(pk(...))`, `prove(hashlock(...))`, `state(older, ...)`, `state(after, ...)`. in particular `sha256(H)` at boolean position IS the hashlock fragment — load-bearing wire compatibility, since every `completion_script` the protocol writes on a transfer lock is spelled `sha256(<hex>)` (DEP-09, DEP-13, the Lightning bridge in DEP-10). hash digests accept both the canonical `0x`-prefixed form and bare hex; the per-function digest-length requirement (32 or 20 bytes) keeps the bare form unambiguous.

miniscript's `pk_h` IS provided: the witness remains keyed by key (not hash-of-key), and the verifier discharges `pk_h(H)` by finding a witness key that both hashes to `H` under hash160 and carries a valid signature over the operation. the spender thus reveals the key inside the witness, exactly as miniscript's `pk_h` reveals it in the script witness; conformance vector `05_pk_h_satisfied` pins the discharge.
